...
public class StressTestFileServiceImpl implements StressTestFileService {
    ...
    @Autowired
    private static StressTestFileDao stressTestFileDao;    
    @Autowired
    public StressTestFileServiceImpl(StressTestFileDao stressTestFileDao) {
    	StressTestFileServiceImpl.stressTestFileDao = stressTestFileDao;
    }
...
//        new JavassistEngine().fixJmeterStandrdEngine();
    ...
    @Override
    public List<StressTestFileEntity> querycsvdata(Integer iscsvdata) {
        return stressTestFileDao.querycsvdata(iscsvdata);
    }
    
    @Override
    public int querycsvdataTotal() {
        return stressTestFileDao.querycsvdataTotal();
    }
    
    @Override
    public List<StressTestFileEntity> queryslaveFile(Long[] membershipfileid) {
        return stressTestFileDao.queryslaveFile(membershipfileid);
    }
    ...
    public String runSingle(Long fileId) {
        ...
        logger.info("--------"+stressTestUtils.getJmeterHome());
        String casePath = stressTestUtils.getCasePath();
...
//                Object engine = engineRun(stressTestFile, jmxTree);
    ...
    public void synchronizeFile(Long[] fileIds) {
        ...
        for (StressTestSlaveEntity slave : stressTestSlaveList) {
        	...
        	synchronizeSingleSalve(fileIds, slave);
        }
    ...
    @Override
    @Transactional
    public void synchronizeSingleSalve(Long[] fileIds, Long slaveId) {
    	//查询所有salve服务器
    	StressTestSlaveEntity stressTestSlave = stressTestSlaveDao.queryObject(slaveId);

    	synchronizeSingleSalve(fileIds, stressTestSlave);
    }

    /**
     * 向单个子节点同步参数化文件
     */
    public void synchronizeSingleSalve(Long[] fileIds, StressTestSlaveEntity stressTestSlave) {

        // 不向本地节点传送文件
        if ("127.0.0.1".equals(stressTestSlave.getIp().trim())) {
            return;
        }
        
        SSH2Utils ssh2Util = new SSH2Utils(stressTestSlave.getIp(), stressTestSlave.getUserName(),
        		stressTestSlave.getPasswd(), Integer.parseInt(stressTestSlave.getSshPort()));
        for (Long fileId : fileIds) {
            StressTestFileEntity stressTestFile = queryObject(fileId);
            putFileToSlave(stressTestSlave, ssh2Util, stressTestFile);
        }
    }
...
//        stressTestFile.setStatus(StressTestUtils.RUN_SUCCESS);
        ...
        if (newStressTestFile == null) {
            ...
            //标记上传savle的文件隶属于哪个源文件的(fileid)
            newStressTestFile.setMembershipfileid(stressTestFile.getFileId()); 
    ...
    /**
     * 根据主文件的fileId 删除对应的slave节点所有子文件
     */
    public void deleteSlaveFile(Long fileId) {
        ...
        deleteSlaveFile(fileDeleteList);
    }
    ...
    @Override
    @Transactional
    public void deleteSlaveFile(List<StressTestFileEntity> fileDeleteList) {
        ...
        String slaveIds = "";
        Map<String,String> slaveFilePath = new HashMap<String,String>();
        ...
        for (StressTestFileEntity stressTestFile4Slave : fileDeleteList) {
            ...
            //子文件id及对应路径
            slaveFilePath.put(stressTestFile4Slave.getSlaveId().toString(), stressTestFile4Slave.getFileName());
...
//            ssh2Util.runCommand("rm -f " + getSlaveFileName(stressTestFile, slave));
            if (slaveFilePath.get(slave.getSlaveId().toString()) == null) {
            	continue;
			} else {
				ssh2Util.runCommand("rm -f " + slaveFilePath.get(slave.getSlaveId().toString()));
			}
    ...
    @Override
    @Transactional
    public void updateSlaveFileName(StressTestFileEntity stressTestFileEntity, String finalname) {

    	//查询slave信息
    	StressTestSlaveEntity slave = stressTestSlaveDao.queryObject(stressTestFileEntity.getSlaveId());
    	
    	//修改后的名字(需要带后缀名)
    	String finaFilelname = slave.getHomeDir() + "/bin/stressTestCases/" + finalname;
    	
    	//远程修改slave文件名
        SSH2Utils ssh2Util = new SSH2Utils(slave.getIp(), slave.getUserName(),
                slave.getPasswd(), Integer.parseInt(slave.getSshPort()));
        
		ssh2Util.runCommand("mv " + stressTestFileEntity.getFileName() + " " + finaFilelname);

		//修改sql对应的fileName(删除文件是依靠该字段)
		Map<String, Object> map = new HashMap<>();
		map.put("fileName", finaFilelname);
    ...
    @Override
    @Transactional
    public String getSlaveFileName(StressTestFileEntity stressTestFileEntity) {

    	//查询slave信息
    	StressTestSlaveEntity slave = stressTestSlaveDao.queryObject(stressTestFileEntity.getSlaveId());
    	
    	//修改后的名字(需要带后缀名)
    	String fileurl = slave.getHomeDir() + "/bin/stressTestCases/";
    	
    	return stressTestFileEntity.getFileName().replace(fileurl, "");
